...
class ConvEmbed(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1d = nn.Conv1d(1, 32, 10, stride=4)
        
        self.layer1 = nn.Sequential(
            nn.Conv1d(in_chans, embed_dim, 8, 1, 0),
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        #print("Before block: {}".format(x.shape)) #256, 576, 80
        H=1
        for i, blk in enumerate(self.blocks):
            x = blk(x, H, W)

        #print("After block: {}".format(x.shape)) #256, 576, 80
        if self.cls_token is not None:
...
class ConvolutionalVisionTransformer(nn.Module):
    def __init__(self,input_size,
        ...
        for i in range(self.num_stages):
            kwargs = {
                'patch_size': [7, 3, 3][i],
                'patch_stride': [4, 2, 2][i],
                'patch_padding': [2, 1, 1][i],
                'embed_dim': [120, 180, 240][i],
                'depth': [1, 2, 4][i],
                'num_heads': [1, 3, 6][i],
            ...
            in_chans = [120, 180, 240][i]

        dim_embed = [120, 180, 240][-1]
        self.norm = norm_layer(dim_embed)
        self.cls_token = [False, False, True][-1]

        # Classifier head
        self.head = nn.Linear(dim_embed, self.num_classes) if num_classes > 0 else nn.Identity()
        trunc_normal_(self.head.weight, std=0.02)

        self.relu1 = nn.LeakyReLU(.1)
